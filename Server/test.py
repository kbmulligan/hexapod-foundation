...
NO_SYSTEM_PARAMETER_ERROR_MSG = "Parameter error: Please provide system to test"
ROBOT_SYSTEMS = ("led", "servo", "ultrasonic", "adc", "buzzer")
DELAY_TIME = 0.005
...
def test_Adc():
    ...
    try:
        while True:
            ...
            print ("The battery voltage is " + str(Power) + " (Servo Power, Control Power)\n")
            time.sleep(1)
    except KeyboardInterrupt:
        print("\nEnd of program")
...
if __name__ == '__main__':
    ...
    import sys
    if len(sys.argv) < 2:
        print (NO_SYSTEM_PARAMETER_ERROR_MSG)
        exit() 
    if sys.argv[1] == 'Led':
        test_Led()
        ...
        print(f"Invalid system selected to test. Please select: {ROBOT_SYSTEMS}")
